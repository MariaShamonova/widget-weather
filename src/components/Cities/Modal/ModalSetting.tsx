...
const ModalSetting: FunctionComponent<AppProps> = ({
	open,
	closeModal,
	clickSave,
	selectedList
	...
	const deleteNotCorectSymbol = (value: string) => {
	...
	const addCityInProp = (e: MouseEvent<HTMLDivElement> | KeyboardEvent<HTMLDivElement>) => {
		...
		const existCurrEnterCityInArray = selectedListCities.map(function(e) { return e.name.toUpperCase(); }).indexOf(value.toUpperCase());
	...
	const removeItemCity = (id: number) => {
		//const index = selectedListCities.map(function(e) { return e.id; }).indexOf(id);
		const temp = selectedListCities.filter((el: SelectedListType) => {return el.id !== id})
		console.log(temp)
	...
	const MessageError = useMemo(() => {
		return(
			<div className="add-city__error-message">
				{errorMessage}		
			</div>
		)
	}, [errorMessage]);
